...
const LoginForm: FC = () => {
   const { handleSubmit, control, formState: { errors }, watch } = useForm<ILoginFormData>();
      ...
      <Flex vertical justify='center' align='center' gap='middle'>
         ...
         <Form onFinish={(handleSubmit(onSubmit))} action='#'>
            ...
            <Form.Item label='Пароль' required validateStatus={errors.password ? 'error' : ''} help={errors.password?.message}>
               <Controller name='password' control={control} rules={{ minLength: { value: 6, message: 'Минимальная длина пароля 6 символов' } }} render={({ field }) => 
                  <Input.Password aria-invalid={!!errors.password} {...field} />
               } />
            </Form.Item>
            <Flex justify='center'>
               <Button htmlType='submit' loading={isLogining}>Войти</Button>
            </Flex>
         </Form>
         <Flex>
            {loginStatus?.message &&
               <p className={loginStatus.type === 'error' ? 'failed-response' : 'success-response'}>{loginStatus.message}</p>
            }
         </Flex>
      </Flex>
   )
